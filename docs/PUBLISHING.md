...
- **npm**: trusted publishing, no secret. The package on npmjs.com lists this repository, the workflow `release.yml`, and the environment `release` as its trusted publisher, and the job has `id-token: write`. Do not pass `registry-url` to `setup-node` or set `NODE_AUTH_TOKEN`: `setup-node` then writes an `_authToken` line into `.npmrc`, and any token line, empty or unexpanded, makes npm skip the OIDC exchange and fail with `ENEEDAUTH` or `404`. The `@redline` scope on npm belongs to someone else, so the package is scoped to the user; the bin stays `redline-mcp`. The `files` allowlist ships `src/` and `LICENSE` only.
