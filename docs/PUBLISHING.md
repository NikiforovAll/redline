...
- **npm**: trusted publishing, no secret. The package on npmjs.com lists this repository, the workflow `release.yml`, and the environment `release` as its trusted publisher, and the job has `id-token: write`. Two values are compared case-sensitively against the GitHub token: the owner in the trusted publisher entry, and `repository.url` in `packages/mcp/package.json`, both `NikiforovAll/redline`. A lowercase owner makes npm fall back to no auth (`ENEEDAUTH`); a lowercase url fails the provenance check (`E422`). The step runs `npm publish` from `packages/mcp`. Do not pass `registry-url` to `setup-node` or set `NODE_AUTH_TOKEN`: `setup-node` then writes an `_authToken` line into `.npmrc`, and any token line, empty or unexpanded, makes npm skip the OIDC exchange and fail with `ENEEDAUTH` or `404`. The `@redline` scope on npm belongs to someone else, so the package is scoped to the user; the bin stays `redline-mcp`. The `files` allowlist ships `src/` and `LICENSE` only.
