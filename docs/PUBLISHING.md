...
- **Marketplace**: no secret. A user-assigned managed identity in Azure carries a federated credential for this repo and environment, and the identity is a Contributor on the `nikiforovall` publisher. The job signs in with `azure/login` using the variables `AZURE_CLIENT_ID` and `AZURE_TENANT_ID`, then `vsce publish --azure-credential`. The publisher member is the id that `.github/workflows/marketplace-identity.yml` prints when run by hand, not the identity's client or object id. Azure DevOps retires the PAT route on 2026-12-01.
...
Extension: search for Redline in the Extensions view, or `code --install-extension nikiforovall.redline-extension`. VS Code updates it from then on. The plugin ships a script that installs the version the plugin pins, for a machine where the extension must match a specific server:
