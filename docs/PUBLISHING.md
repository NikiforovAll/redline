...
- **npm**: trusted publishing, no secret. The package on npmjs.com lists this repository, the workflow `release.yml`, and the environment `release` as its trusted publisher, and the job has `id-token: write`. Do not set `NODE_AUTH_TOKEN` in the publish step: `setup-node` writes an `_authToken` line from it into `.npmrc`, and an empty token makes npm fail with `ENEEDAUTH` instead of exchanging the OIDC token. The `@redline` scope on npm belongs to someone else, so the package is scoped to the user; the bin stays `redline-mcp`. The `files` allowlist ships `src/` and `LICENSE` only.
