...
| `get_review`      | `roundId?`, `threads?`, `peek?`, `wait?` (seconds, max 300)     | Returns the submitted comments of a round as markdown and marks them delivered. Without `roundId`, the newest round with undelivered comments. With `peek`, one index line per thread plus only the comments Claude has not seen, no diff context or history; the threads count as delivered all the same. With `wait`, blocks until a submit or the timeout. |
...
| `redline: the VS Code window for <folder> is not answering (stale lock)`         | The window closed without cleanup, the extension crashed, or its server stopped taking connections. The extension checks its server every 10 seconds and restarts it, and each tool call waits up to 45 seconds for that (`REDLINE_DISCOVER_WAIT_MS`) before it shows this error. Run **Developer: Reload Window** in that VS Code window, then retry. The **Redline** output channel logs each failed check. |
